...
(asymmetric), "rsa-4096" (asymmetric), "ml-dsa" (asymmetric), "slh-dsa" (asymmetric), "kyber" (post-quantum KEM) are supported.  Defaults to "aes256-gcm96".
`,
			},
...
key.`,
			...
			"parameter_set": {
				Type: framework.TypeString,
				Description: `The parameter set to use. Applies to ML-DSA, SLH-DSA, and Kyber key types.
...
For Kyber key types, valid values are kyber-512, kyber-768, or kyber-1024.`,
			},
			"hybrid_key_type_pqc": {
				Type: framework.TypeString,
...
func (b *backend) pathPolicyWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	case "kyber":
		...
		if parameterSet != keysutil.ParameterSet_KYBER_512 &&
			parameterSet != keysutil.ParameterSet_KYBER_768 &&
			parameterSet != keysutil.ParameterSet_KYBER_1024 {
			return logical.ErrorResponse(fmt.Sprintf("invalid parameter set %s for key type %s", parameterSet, keyType)), logical.ErrInvalidRequest
		}
...
func (b *backend) formatKeyPolicy(p *keysutil.Policy, context []byte) (*logical.Response, error) {
	...
	case keysutil.KeyType_ECDSA_P256, keysutil.KeyType_ECDSA_P384, keysutil.KeyType_ECDSA_P521, keysutil.KeyType_ED25519, keysutil.KeyType_RSA2048, keysutil.KeyType_RSA3072, keysutil.KeyType_RSA4096, keysutil.KeyType_ML_DSA, keysutil.KeyType_HYBRID, keysutil.KeyType_SLH_DSA, keysutil.KeyType_KYBER:
		...
		for k, v := range p.Keys {
			...
			case keysutil.KeyType_KYBER:
				key.Name = "kyber-" + strings.TrimPrefix(p.ParameterSet, "kyber-")
			}
